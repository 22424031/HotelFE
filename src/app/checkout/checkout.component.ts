...
export class CheckoutComponent implements OnInit {
  ...
  payCash(){
    console.log('cash')
   /*  var payLoad = {
       startDate : this.reservationGet.startDate,
       endDate : this.reservationGet.endDate,
       roomId : this.reservationGet.roomId,
      name: this.bookForm.controls['name'].value,
      email: this.bookForm.controls['email'].value,
      phoneNumber: this.bookForm.controls['phoneNumber'].value,
      address: this.bookForm.controls['address'].value,
    ...
    this.http.post<any>(environment.BASE_URL_API + '/user/invoice/create', payloadCash, { headers }).subscribe(
      (response) => {
        const redirectUrl = '/paymentCash';
        if (redirectUrl) {
